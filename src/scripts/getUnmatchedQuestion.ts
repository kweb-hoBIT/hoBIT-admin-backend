            ...
            content: `
              ...
              ## **🔥 중요 규칙**
                ...
                1. **유사한 질문을 매칭하기**
                  ...
                  - **완전히 일치하지 않더라도 다음 조건 중 하나라도 만족하면 같은 질문으로 간주**하고 매칭해야 합니다:
                    - 의미적으로 동일한 질문 (예: "학점교류 학점 인정 절차가 궁금해요" vs "교류 학점 인정 방법 알려줘")
                    - 일부 표현이 다르지만 같은 의미인 경우 (예: "수업 신청 못했어요" vs "수강신청 추가 가능할까요?")
                    - 문장 길이가 다르지만 핵심 단어가 포함된 경우 (예: "포털에서 이름 바꾸고 싶어요" vs "개명해서 이름 변경하고 싶어요")
                  - 질문 리스트(userQuestion) 또는 기존 FAQ(question_ko)에 포함된 질문이 여러 문장으로 이루어져 있더라도, 그 중 하나의 문장이라도 의미상 유사한 부분이 있다면 매칭됩니다.

                2. 매칭된 질문은 **제외하고**, 매칭되지 않은 질문만 unmatched로 반환
                  - userQuestion에 있는 질문이 question_ko의 어떤 질문과도 매칭되지 않는다면 unmatched로 반환합니다.
                  - **어떤 질문과도 유사하지 않은 질문만 포함**해야 합니다.
