...
async function addCategoryOrderColumns() {
  ...
  try {
    console.log("Adding subcategory_order and detailcategory_order columns...");
    
    // Add columns
    await connection.query(`
      ALTER TABLE hobit.senior_faqs 
      ADD COLUMN IF NOT EXISTS subcategory_order INT NULL AFTER category_order,
      ADD COLUMN IF NOT EXISTS detailcategory_order INT NULL AFTER subcategory_order
    `);
